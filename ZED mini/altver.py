...
stereo = cv2.StereoSGBM_create(
    ...
    blockSize=13,  # Adjust for better noise reduction
    P1=8 * 3 * 13 ** 2,  # Penalty on disparity change
    P2=32 * 3 * 13 ** 2,
    disp12MaxDiff=1,
...
while True:
    ...
    disparity_display = cv2.equalizeHist(disparity_display)

    # Apply a colormap for better visualization
    disparity_colormap = cv2.applyColorMap(disparity_display, cv2.COLORMAP_JET)
